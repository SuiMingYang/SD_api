...
def data_set(fname):
    '''
        ...
        删除包含空值的行
    '''
    data = pd.read_csv(fname,"\t", skiprows=[0],names=['area','indicator','value'])
    ...
    area_list = [str(x) for x in area_list]
...
def sd_pca(fname,components,result_name):
    ...
    area_value = pca.transform(values)

    value_list = area_value.round(decimals=4).tolist()

    result_dict["score"] = np.column_stack((area_list,value_list)).tolist()

    area_value = pd.DataFrame(area_value, index=area_list)
    ...
    if result_name:
        fout.write("\n===============================\n")
        fout.close()
        area_value.to_csv(result_name,sep="\t",float_format='%8.4f',mode="a",header=None)
